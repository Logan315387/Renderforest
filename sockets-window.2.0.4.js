...
function subscribeToSockets (cb) {
  ...
  if (userId && socketUrl) {
    ...
    activeSocketConnections.push(ioEvents);
    return ioEvents;
  }
}
...
function disconnectSockets (socketId) {
  activeSocketConnections = activeSocketConnections.reduce(function(_socketConnections, _socket ) {
    if ((!socketId || _socket.id === socketId) && typeof _socket.disconnect === 'function') {
      _socket.disconnect();
      return _socketConnections;
    }
    _socketConnections.push(_socket);
    return _socketConnections;
  }, []);
}
